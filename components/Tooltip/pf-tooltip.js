...
import { pfUtil } from '../PfUtil/pf-util';

/**
 * <b>&lt;pf-tooltip&gt;</b> element for Patternfly Web Components
 *
 * @example {@lang xml}
 * <pf-tooltip animation="fade" targetSelector="#btn-left" placement="left" delay="100"
 *  duration="150" containerSelector="#container"></pf-alert>
 *
...
export class PfTooltip extends HTMLElement {
  ...
  init() {
    ...
    this._placement = this.getAttribute('placement')
      ...
      : 'right';
    this._delay = parseInt(this.getAttribute('delay'), 10) || 100;
    this._mouseHover = 'onmouseleave' in document
      ? ['mouseenter', 'mouseleave']
      : ['mouseover', 'mouseout'];
    this._tipPositions = /\b(top|bottom|left|top)+/;
    this._duration = pfUtil.isMSIE && pfUtil.isMSIE < 10
      ? 0
      : parseInt(this.getAttribute('duration'), 10) || 150;
    this._containerSelector = this.getAttribute('containerSelector');
    this._container = this._containerSelector
      ? document.querySelector(this._containerSelector)
      : document.body;
  ...
  _removeTooltip() {
    if (this.tooltip) {
      this._container.removeChild(this.tooltip);
    }
    this.tooltip = null;
  }
  ...
  _createTooltip() {
    ...
    const tooltipInner = clone.querySelector('.tooltip-inner');
    ...
    const tooltips = this._container.querySelectorAll('.tooltip');
   ...
   */
  _showTooltip() {
    if (!/\bin/.test(this.tooltip.className)) {
      pfUtil.addClass(this.tooltip, 'in');
    }
  }
}
(function () {
