  ...
  render() {
    return (
      <ListExpansionView key="list-expansion-view">
        ...
        <div className="infra-group-item list-group-item list-view-pf-stacked list-view-pf-top-align" key={i} onClick={(e) => this.listItemClick(e, i)}>
          ...
          <div className="list-view-pf-main-info">
            <div className="list-view-pf-left">
              <span className={ this.getIcon(infrastructure.status)}></span>
              <img style={{height: 40, marginRight: 15, marginLeft: 15, marginTop: -5}} src={infraImage(infrastructure.provider)} />
            </div>
            <div className="list-view-pf-body">
              <div className="list-view-pf-description">
                ...
                <div className="list-group-item-text">
                  {(() => {
                    let content = [];
                    if(infrastructure.status === constants.ANSIBLE_JOB_STATUS.SUCCESSFUL 
                    ...
                    else if (infrastructure.status === constants.ANSIBLE_JOB_STATUS.PENDING 
          ...
          <div className="row">
            <div className="col-xs-12 col-sm-6 col-md-4">
              <dl className="dl-horizontal">
                <dt>Ansible</dt>
                <dd><a href={ infrastructure.ansible_tower_link } target="_blank">Tower Job</a></dd>
            ...
            <div className="col-xs-12 col-sm-6 col-md-4">
              <dl className="dl-horizontal">
                <dt>Started</dt>
                <dd>{ moment(infrastructure.datetime_started).format('MMM Do YYYY, h:mm:ss a') }</dd>
                {infrastructure.status !== constants.ANSIBLE_JOB_STATUS.PENDING &&
                <dt>Finished</dt>
                }
                {infrastructure.status !== constants.ANSIBLE_JOB_STATUS.PENDING &&
                <dd>{ moment(infrastructure.datetime_completed).format('MMM Do YYYY, h:mm:ss a') }</dd>
                }
              </dl>
            </div>
            <div className="col-xs-12 col-sm-6 col-md-4">
              <dl className="dl-horizontal">
                <dt>Status</dt>
                <dd className={cx({
                    'text-danger': infrastructure.status === constants.ANSIBLE_JOB_STATUS.FAILED 
                      || infrastructure.status === constants.ANSIBLE_JOB_STATUS.CANCELLED, 
                    'text-success': infrastructure.status === constants.ANSIBLE_JOB_STATUS.SUCCESSFUL
                  })}>{infrastructure.status}</dd>                  
              </dl>
            </div>
          </div>
          <div className="row">
            <div className="col-xs-12 col-sm-6 col-md-4">
              <dl className="dl-horizontal">
                <dt>PaaS</dt>
                <dd><img src="/img/OpenShift-logo.svg" style={{height: 100}}/></dd>
              </dl>
            </div>
            <div className="col-xs-12 col-sm-6 col-md-4">
              <dl className="dl-horizontal">
