class Wizard {
  constructor(id) {
    ...
    this.updateTabGroup = () => {
      ...
      $(
        `${self.modal} .wizard-pf-step[data-tabgroup='${self.currentGroup}']`
      ).addClass('active');
    ...
    this.updateActiveTab = () => {
      $(
        `${self.modal} .list-group-item[data-tab='${self.currentTab}']`
      ).addClass('active');
    ...
    this.tabGroupSelect = () => {
      $(`${self.modal} .wizard-pf-step>a`).click(function () {
        ...
        if (self.currentTab === undefined) {
          self.currentTab = $(
            `${self.modal} .list-group[data-tabgroup='${self.currentGroup}'] .list-group-item:first-child`
          ).data('tab');
    ...
    this.tabSelect = () => {
      $(
        `${self.modal} .wizard-pf-sidebar .list-group-item>a`
      ).click(function () {
    ...
    this.wizardPaging = (direction) => {
      ...
      if (
        newTab !== Math.round(10 * (direction * 0.1 + self.currentTab)) / 10
      ) {
    ...
    this.init = (modalId) => {
      ...
      $(`${self.modal} .wizard-pf-sidebar .list-group-item`).each(function () {
        ...
        $(this).attr(
          'data-tab',
          $(this).parent().index() + 1 + ($(this).index() / 10 + 0.1)
        );
      });
      ...
      self.tabs = $(
        `${self.modal} .wizard-pf-sidebar .list-group-item`
      ).map(function () {
      ...
      $(`${self.modal} .wizard-pf-sidebar .list-group-item.active`).removeClass(
        'active'
      );
      // apply active class to new current tab and associated contents
      self.updateActiveTab();
