...
class UsersPage extends React.Component {
  ...
  getGroups() {
    ...
    const groups = [
      {
        id: 1,
        name: 'loyalty-developers',
        users: [{ id: 1, username: 'jholmes' }, { id: 2, username: 'priley' }]
      },
      {
        id: 2,
        name: 'loyalty-admins',
        users: [
          { id: 1, username: 'jholmes' },
          { id: 2, username: 'priley' },
          { id: 3, username: 'jlabocki' }
        ]
      },
      {
        id: 3,
        name: 'loyalty-users',
        users: [{ id: 4, username: 'vconzola' }]
      },
      { id: 4, name: 'cloud-admins', users: [{ id: 1, username: 'jholmes' }] },
      {
        id: 5,
        name: 'cloud-users',
        users: [{ id: 2, username: 'priley' }, { id: 3, username: 'jlabocki' }]
      },
      { id: 6, name: 'cloud-viewers', users: [] }
    ];

    // for table visualization we join groups into a comma separated string
    const groupsJoined = groups.map((g) => {
      const users = g.users.map(u => u.username);
      return { name: g.name, users: users.join() };
    });

    this.setState({ groups, groupsJoined });
  }
  ...
  handleUserFilterChanged = (event, filter, filterText) => {
    event.preventDefault();
    this.setState({
      usersFilterColumn: filter.field,
      usersFilterText: filterText
    });
  };
  ...
  handleUserSearch = (event) => {
    this.setState({
      usersFilterColumn: '*',
      usersFilterText: event.target.value
    });
  };
  ...
  handleGroupFilterChanged = (event, filter, filterText) => {
    event.preventDefault();
    this.setState({
      groupsFilterColumn: filter.field,
      groupsFilterText: filterText
    });
  };
      ...
      <Layout className="container-fluid container-pf-nav-pf-vertical" nav>
        {(() => {
          if (this.state.createUserView) {
            ...
            return (
              <CreateGroupForm handleSubmit={this.handleCreateGroupSubmit.bind(this)}
          ...
          if (this.state.users.length) {
            ...
            userTab = [
              <div className="blank-slate-pf table-view-pf-empty"
                key="no-users-div">
          ...
          if (this.state.groups.length) {
            ...
            groupTab = [
              <ToolbarView key="groups-toolbar"
                primaryAction={{
                  label: 'Create Group',
                  action: this.handleCreateGroup.bind(this)
                }}
                columns={columns}
          ...
          return [
            ...
            <Tabs key="pf-tabs"
              tabChanged={this.handleTabChanged.bind(this)}
              tabsClass="nav nav-tabs user-tabs">
