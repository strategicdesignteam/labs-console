...
var isAuthenticated = function(req) {
  ...
  return !user || user.name !== username || user.pass !== password || req.headers.hash !== expectedHash
    ? false: true;
}
...
module.exports.login = function(req, res, next) {
  ...
  if(req.body.username === username && req.body.password === password){
    ...
    res.json({ username: req.body.username, password: req.body.password, hash: hash});
  } else {
    res.send(401);
  }
}